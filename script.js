...
function changeLights() {
  ...
  setTimeout(() => {
    ...
    document.getElementById("durationValue").textContent =
      simulation.settings.greenDuration;
    document.getElementById("greenDuration").value =
      simulation.settings.greenDuration;
...
function moveCarThroughIntersection(car, direction) {
  ...
  const speed = 6; // seconds to cross
  ...
  switch (direction) {
    case "west":
      start = { left: "-50px", top: "50%" };
      end = { left: "110%", top: "50%" };
      break;
    case "east":
      start = { left: "110%", top: "55%" };
      end = { left: "-50px", top: "55%" };
      break;
    case "north":
      start = { top: "-40px", left: "48%" };
      end = { top: "110%", left: "48%" };
      carElement.style.transform = "rotate(90deg)";
      break;
    case "south":
      start = { top: "110%", left: "52%" };
      end = { top: "-40px", left: "52%" };
      carElement.style.transform = "rotate(90deg)";
      break;
  }

  carElement.style.left = start.left;
  carElement.style.top = start.top;

  movingCars.appendChild(carElement);

  setTimeout(() => {
    carElement.style.transition = `all ${speed}s linear`;
    carElement.style.left = end.left;
    carElement.style.top = end.top;
...
function updateAI() {
  ...
  let decisionText =
    "<p class='mb-2 font-semibold text-green-400'>AI Decision Process:</p>";
  decisionText += "<div class='bg-gray-800 rounded p-3'>";

  // Calculate densities
  const densities = Object.entries(queues)
    .map(([dir, cars]) => ({
      direction: dir,
      count: cars.length,
    }))
...
function updateUI() {
  // Update queue counts
  document.getElementById("westCount").textContent =
    simulation.queues.west.length;
  document.getElementById("eastCount").textContent =
    simulation.queues.east.length;
  document.getElementById("northCount").textContent =
    simulation.queues.north.length;
  document.getElementById("southCount").textContent =
    simulation.queues.south.length;

  // Update statistics
  const avgWait =
    simulation.stats.carsProcessed > 0
      ? Math.floor(
          simulation.stats.totalWaitTime / simulation.stats.carsProcessed,
        )
      : 0;

  document.getElementById("avgWaitTime").textContent = `${avgWait}s`;
  document.getElementById("carsProcessed").textContent =
    simulation.stats.carsProcessed;
  document.getElementById("totalCars").textContent = simulation.stats.totalCars;
  document.getElementById("carsThrough").textContent =
    simulation.stats.carsProcessed;
  ...
  document.getElementById("efficiencyScore").textContent =
    `${Math.max(0, Math.floor(efficiency))}%`;
...
function updateQueueUI(direction) {
    ...
    cars.forEach((car, index) => {
        ...
        carElement.style.margin = "3px auto";
